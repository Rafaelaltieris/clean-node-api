...
describe('Login Controller', () => {
  test('Should return 400 if no email priovided', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        password: 'any_password'
  ...
  test('Should return 400 if no password is priovided', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        email: 'any_email@email.com'
  ...
  test('Should return 400 if an invalid email is priovided', async () => {
  ...
  test('Should call Authentication with with correct values', async () => {
